...
## Project conventions
- **Google account:** this project runs entirely under **`will@whizzy.org`**, not the Chainguard account. `clasp login` uses whizzy.org; the Drive folder, the Apps Script project, and the deployment all live there. Don't mix accounts.
- **Drive root folder ID:** intentionally not committed to this repo. The actual value lives in Claude's project memory (`~/.claude/projects/-home-will-source-bookcast/memory/bookcast-deployment-ids.md`) and at runtime in Apps Script `PropertiesService` under key `ROOT_FOLDER_ID`. Treat it like any other URL secret — don't paste into PR descriptions, issues, or chat.
- **The Drive folder is the source of truth, at every moment.** Top-level is the user's audiobook root. Subfolders = books. MP3 files in a book folder = chapters, ordered by filename. Optional `metadata.json` per book folder; the cover is auto-detected (see below). See [DESIGN.md §3](./DESIGN.md).
...
1. Create a Drive folder for audiobooks (e.g. `Audiobooks/`) **in the whizzy.org account**, and keep it private — do NOT link-share it (see Hard rules). Drop 1–2 short test books in it with the layout from DESIGN.md §3. A LibriVox public-domain audiobook is the easy test case. Note the folder's Drive ID (the long string in the URL when you open the folder in Drive); record it in Claude's project memory rather than committing it to this repo. Cover image filename does not matter; the script picks the largest image in the folder. `metadata.json` is optional; if absent, the script will auto-populate it from Open Library on first scan.
2. Install clasp and create the Apps Script project (sign in with whizzy.org when the browser opens):
   ```
   npm install -g @google/clasp
   clasp login                                                # opens browser — choose will@whizzy.org
   clasp create --type webapp --rootDir ./src --title bookcast
   clasp push                                                 # uploads stubs to Apps Script
   ```
   Also enable the Apps Script API at https://script.google.com/home/usersettings for the whizzy.org account before `clasp create`, or it'll fail with an unhelpful error.
...
- **2026-05-16** — Runs under `will@whizzy.org`, not the Chainguard account. Reason: this is personal media, not work; the script's full-Drive access scope should live on a personal account so it can't accidentally interact with Chainguard data. The whizzy.org account also owns the Drive folder, the Apps Script project, and the deployment.
